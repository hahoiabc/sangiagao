...
- **Lịch sử:** 1.6.6(52) đã duyệt · **1.6.7 BỎ QUA** (lỡ dùng trên Apple) · 1.6.8(53) · **1.6.9(54)** đã XUẤT BẢN công khai Google Play (04/07) · **1.6.10(55)** gửi CẢ 2 nền tảng (05/07) · **1.6.11(56)** AAB fix Google Play Billing 8.0.0 (chỉ Android) · **1.6.12(57)** gửi CẢ 2 nền tảng (20/08): edge-to-edge (Android 15/16) + bảo mật đổi SĐT (verify OTP) + Trang Nhà phát triển + thông báo lỗi login. **Android ĐÃ LIVE. iOS: Apple TỪ CHỐI 2 LẦN (Guideline 3.1.2)** → fix metadata + nộp lại cùng build 57 (chờ duyệt).
- Số kế tiếp khi build: **1.6.13+58**.

### ⚠️ BẪY Apple 3.1.2 — subscription cần link EULA Ở METADATA (không chỉ trong app)
App có gói tự-gia-hạn: Apple đòi link **Terms of Use (EULA)** functional **TRÊN TRANG SẢN PHẨM App Store**, KHÔNG chỉ trong màn mua. `subscription_screen.dart` đã có sẵn tiết lộ tự-gia-hạn + link Điều khoản/Bảo mật (đủ phần in-app) → nhưng vẫn bị từ chối vì **thiếu link ở metadata**. **FIX ĐÚNG (thuần metadata, KHÔNG build lại):** dán vào ô **Description** ĐÚNG link EULA tiêu chuẩn Apple **`https://www.apple.com/legal/internet-services/itunes/dev/stdeula/`** + Privacy Policy URL. **BẪY (đã dính):** dán link web riêng (`sangiagao.vn/dieu-khoan-su-dung`) vào Description → bộ quét tự động **KHÔNG nhận** → từ chối lại. Phải là link `apple.com/.../stdeula/` (EULA tiêu chuẩn) HOẶC set Custom EULA trong App Information → License Agreement. Sau đó reply Resolution Center + Add for Review giữ build. Áp dụng cho MỌI app 365 có IAP.
