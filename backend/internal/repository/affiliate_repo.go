...
// --- commission_records ---
// INSERT đã chuyển vào commission_engine.RecordForPayment (cần chạy trong tx
// chung với SELECT FOR UPDATE để serialize payment_sequence). Repo còn read +
// stats helpers thôi.
...
func (r *AffiliateRepo) ListRecordsForReferrer(ctx context.Context, userID string, limit, offset int) ([]*model.CommissionRecord, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT id, referrer_user_id, referee_user_id, subscription_id,
		        ...
		        base_amount, stage, rate, commission_amount, payment_sequence, rule_id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&rec.BaseAmount, &rec.Stage, &rec.Rate, &rec.CommissionAmount, &rec.PaymentSequence, &rec.RuleID,
